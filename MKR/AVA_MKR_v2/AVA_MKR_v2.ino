...
int batlife;                        //remaining battery life
int savedt;                         //time read off of the MKR memory
int etime;                          //elapsed time to calculate remaining battery
...
void setup() {
  ...
  pinMode(A3,INPUT);                                // battery monitor pin
  Alarm.timerRepeat(60, Repeats);                   // battery timer
}

void loop() {

  int state = analogRead (A3);                      //reads the value from pin A3
  Alarm.delay(0);                                   //starts the timer for the battery
      ...
      if (client.available()) {                     // if there are bytes to read from the client,
        ...
        if (in == '\n') {                           // check if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.print("Click <a href=\"/B\">here</a> to request battery life <br>");
        ...
        if (currentLine.endsWith("GET /B")) {
          ...
          savedt = etimer.read();                   //read the elapsed time from the MKR
          batlife = ((601-savedt)/6);               //calculate remaining battery life
          if (batlife <0) {                         //avoid a negative battery life
            batlife = 0;  
          }      
          //client.println(savedt);
          client.println(batlife);                  //displays the remaining battery life
...
void Repeats() {
  //writes the elapsed time to the MKR memory every minute
  int state = analogRead (A3);
  if (state > 2000) {                      //if usb is plugged in, timer resets
            float reset = 0;
            etimer.write(reset);
  }
  if (state < 2000) {                      //if usb is unplugged, the elapsed time is incremented
            savedt = etimer.read();
            etime = savedt + 1;
            etimer.write(etime);
